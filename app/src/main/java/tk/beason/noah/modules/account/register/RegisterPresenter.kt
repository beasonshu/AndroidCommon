 ...
 * 注册的Presenter
 */
internal class RegisterPresenter(private val mView: tk.beason.noah.modules.account.register.RegisterContract.View) :
    tk.beason.noah.modules.account.register.RegisterContract.Presenter {
    ...
    override fun register(context: Context, name: String, verifyCode: String, password: String, ensurePassword: String) {
        var errorMessage = tk.beason.noah.manager.UserManager.checkAccount(context, name)
        ...
        errorMessage = tk.beason.noah.manager.UserManager.checkPassword(context, password)
        if (!TextUtils.isEmpty(errorMessage)) {
            mView.showError(errorMessage, true, 0)
            return
        }

        errorMessage = tk.beason.noah.manager.UserManager.checkEnsurePassword(context, password, ensurePassword)
        if (!TextUtils.isEmpty(errorMessage)) {
            mView.showError(errorMessage, true, 0)
            return
        }
        ...
        Http.post()
                .url(tk.beason.noah.constant.Url.register())
